...
const Gallery = () => {
  ...
  const [deleted, setDeleted] = useState(models)
  /* const axios = require('axios'); */
  // instance
  const mock = axios.create({
    baseURL: "http://localhost:3001/model"
  });
...
const handleBlur = () => {
  ...
  const deleteData = async(id) => {
    await axios.delete(`http://localhost:3001/model/${id}`)
    /* setFilters(models.filter((model) => model.id !== id)) */
    .then(response => console.log('Model deleted!',response))
    .catch(err => console.log(err))
    setModels((prevMode)=>prevMode.filter((model) => model.id !== id))
  }
